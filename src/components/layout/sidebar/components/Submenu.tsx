...
const Submenu = ({ children, title = '', icon, borderRadius = '8px', textFontSize = '14px', disabled = false }: SubmenuProps) => {
	...
	return (
		<Box>
			<ListItemStyled onClick={handleClick} sx={{ display: 'flex', gap: '15px' }}>
				<ListIConStyled style={{ minWidth: '0px' }}>{icon ? icon : <CircleOutlined />}</ListIConStyled>
